...
public class WordLadder implements DirectedGraph<String> {
     ...
     */
    public List<DirectedEdge<String>> outgoingEdges(String w) { // 2): Implementing this method.
        /******************************
         * TODO: Task 2               *
        ...
        for(String word:dictionary){ // For every word in our dictionary
            if(w1.length==word.length()) { // Only words with the same number of letters can have an edge
                int count = 0;
                int point = 0;
                for (char c : w1) {
                    if (c == word.toCharArray()[count]) {
                        point++;
                    }
                    count++;
                }
                if(count-point==1){list.add(new DirectedEdge<String>(w, word, 1));} // Add the word to the list iff.
            }                                                                              // the words differ with exactly one letter
        }
        return list;// return list of words that differs with one letter
    }
    ...
    public double guessCost(String w, String u) {
        ...
        for (char c : w.toCharArray()) {
            if (c == u.toCharArray()[count]) {
                point++;
            }
            count++;
        }
        return count-point;//return amount of different letters
    }
